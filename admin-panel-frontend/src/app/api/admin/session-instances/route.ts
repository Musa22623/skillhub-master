...
import { fetchSessionInstancesListPage } from "@/app/lib/api/products.api";
...
export async function GET(request: NextRequest) {
  ...
  if (shouldProxyAdminRequest()) {
    return proxyAdminGet("/admin/session-instances", {
      query: query.query,
      status: query.status,
      sortBy: query.sortBy,
  ...
  const payload = await fetchSessionInstancesListPage(query);
