...
import { fetchMockAllInstructorsListPage } from "@/app/lib/api/users.mock";
...
export async function GET(request: NextRequest) {
  ...
  if (shouldProxyAdminRequest()) {
    return proxyAdminGet("/admin/instructors", {
      query: query.query,
      status: query.status,
      sortBy: query.sortBy,
  ...
  const payload = await fetchMockAllInstructorsListPage(query);
