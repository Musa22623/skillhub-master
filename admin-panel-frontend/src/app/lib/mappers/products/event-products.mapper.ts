...
function mapEventProductRow(row: CatalogProductRecordDto): AdminRecord {
  return { ...row };
}

export function mapEventProductsListResponse(response: EventProductListApiResponse): EventProductPageData {
  return {
    stats: response.stats.map(mapProductStat),
    rows: response.rows.map(mapEventProductRow),
    pagination: response.pagination,
    query: response.query,
  };
}

export function mapEventProductsSummaryResponse(response: EventProductSummaryApiResponse): EventProductSummaryData {
  return {
    stats: response.stats.map(mapProductStat),
  };
}
